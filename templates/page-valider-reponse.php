...
if ($statut_existant !== 'resolue') {
    if ($statut_existant) {
        $wpdb->update($statuts_table, ['statut' => 'resolue'], ['user_id' => $user_id, 'enigme_id' => $enigme_id]);
    } else {
        $wpdb->insert($statuts_table, ['user_id' => $user_id, 'enigme_id' => $enigme_id, 'statut' => 'resolue']);
    }
}

// Envoyer le mail de notification au joueur
if (function_exists('envoyer_mail_notification_joueur')) {
    envoyer_mail_notification_joueur($user_id, $enigme_id, 'bon');
}
